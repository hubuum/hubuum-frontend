	...
	if ("properties" in schema) {
		...
		if (!isRecord(properties)) {
			...
			for (const [propertyName, propertySchema] of Object.entries(properties)) {
				...
				validateSchemaShape(
					propertySchema,
					formatJsonPath(path, propertyName),
					issues,
				);
			}
		}
...
function validateAgainstSchema(
	...
	if (
		types.length > 0 &&
		!types.some((type) => valueMatchesType(value, type))
	) {
		issues.push({
			path,
	...
	if (isRecord(value)) {
		const propertySchemas = isRecord(schema.properties)
			? schema.properties
			: null;
	...
	try {
		...
		const fallbackLineColumn =
			extracted.position !== null
				? getLineAndColumn(input, extracted.position)
...
export function summarizeJsonDocument(value: unknown): string[] {
	if (Array.isArray(value)) {
		return [
			`Valid JSON: array with ${value.length} item${value.length === 1 ? "" : "s"}.`,
		];
	}
	...
	if (isRecord(value)) {
		const keys = Object.keys(value);
		const summary = [
			`Valid JSON: object with ${keys.length} key${keys.length === 1 ? "" : "s"}.`,
		];
		if (keys.length > 0) {
			summary.push(
				`Top-level keys: ${keys.slice(0, 8).join(", ")}${keys.length > 8 ? ", ..." : ""}`,
			);
		}
		return summary;
	}
...
export function analyzeJsonSchema(schema: unknown): JsonSchemaAnalysis {
	...
	if (properties.length > 0) {
		summary.push(
			`Properties: ${properties.slice(0, 8).join(", ")}${properties.length > 8 ? ", ..." : ""}`,
		);
	}
