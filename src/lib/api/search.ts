...
function normalizeSearchResponse(payload: unknown, fallbackQuery: string): UnifiedSearchResponse {
  ...
  return {
    ...
    results: {
      namespaces: Array.isArray(response.results?.namespaces) ? (response.results.namespaces as Namespace[]) : [],
      classes: Array.isArray(response.results?.classes) ? (response.results.classes as HubuumClassExpanded[]) : [],
      objects: Array.isArray(response.results?.objects) ? (response.results.objects as HubuumObject[]) : []
    },
    next: {
      namespaces: typeof response.next?.namespaces === "string" ? response.next.namespaces : null,
      classes: typeof response.next?.classes === "string" ? response.next.classes : null,
      objects: typeof response.next?.objects === "string" ? response.next.objects : null
    }
  };
}
...
export async function fetchUnifiedSearch(params: UnifiedSearchParams, options?: RequestInit): Promise<UnifiedSearchResponse> {
  ...
  if (response.status !== 200) {
    throw new Error(getApiErrorMessage(payload, "Failed to run unified search."));
  }
