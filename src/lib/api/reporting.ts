...
export type StoredReportContentType = Exclude<ReportContentType, "application/json">;
...
export async function listReportTemplates(cursor?: string | null): Promise<ReportTemplatePage> {
  ...
  if (response.status !== 200) {
    throw new Error(getApiErrorMessage(response.data, "Failed to load report templates."));
  }
...
export async function createReportTemplate(payload: NewReportTemplate): Promise<ReportTemplate> {
  ...
  if (response.status !== 201) {
    throw new Error(getApiErrorMessage(response.data, "Failed to create report template."));
  }
...
export async function updateReportTemplate(templateId: number, payload: UpdateReportTemplate): Promise<ReportTemplate> {
  ...
  if (response.status !== 200) {
    throw new Error(getApiErrorMessage(response.data, "Failed to update report template."));
  }
...
export async function deleteReportTemplate(templateId: number): Promise<void> {
  ...
  if (response.status !== 204) {
    throw new Error(getApiErrorMessage(response.data, "Failed to delete report template."));
  }
}
