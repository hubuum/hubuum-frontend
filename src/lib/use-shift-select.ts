...
export function useShiftSelect<T>({ items, selectedIds, setSelectedIds, getId }: UseShiftSelectOptions<T>) {
  ...
  const handleClick = useCallback(
    (itemId: number, checked: boolean, shiftKey: boolean) => {
      ...
      if (!shiftKey || lastClickedIndexRef.current === null) {
        ...
        if (checked) {
          setSelectedIds(selectedIds.includes(itemId) ? selectedIds : [...selectedIds, itemId]);
        } else {
