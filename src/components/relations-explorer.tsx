...
async function fetchConnectedClasses(
	...
	const response = await fetch(
		`/api/v1/classes/${classId}/related/classes?limit=250&sort=path.asc,id.asc`,
		{
			credentials: "include",
		},
	);
...
async function fetchRelatedObjects(
	...
	const response = await fetch(
		`/api/v1/classes/${classId}/objects/${objectId}/related/objects?limit=250`,
		{
			credentials: "include",
		},
	);
...
async function fetchDirectObjectRelations(
	...
	const response = await fetch(
		`/api/v1/classes/${classId}/objects/${objectId}/related/relations?limit=250`,
		{
			credentials: "include",
		},
	);
...
export function RelationsExplorer({ mode }: RelationsExplorerProps) {
	...
	const classDirectRelations = Array.isArray(classRelationsQuery.data)
		? classRelationsQuery.data
		: [];
	...
	const createClassRelationMutation = useMutation({
		mutationFn: async (payload: {
			...
			const response = await fetch(
				`/api/classes/${payload.sourceClassId}/relations`,
				{
					method: "POST",
					headers: {
		...
		<div className="stack">
			...
			{isClassMode ? (
				<div className="card table-wrap">
					...
					{parsedSourceClassId === null ? (
						<div className="muted">
							Select a class to load class-level relations.
						</div>
					) : classRelationsView === "direct" ? (
						classRelationsQuery.isLoading ? (
							...
							<div className="muted">
								No direct class relations for this class.
							</div>
						) : (
