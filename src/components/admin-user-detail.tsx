...
export function AdminUserDetail({ userId }: AdminUserDetailProps) {
	...
	const updateMutation = useMutation({
		mutationFn: async (payload: UpdateUser) => {
			...
			if (response.status !== 200) {
				throw new Error(
					getApiErrorMessage(response.data, "Failed to update user."),
				);
			}
	...
	const deleteMutation = useMutation({
		mutationFn: async () => {
			...
			if (response.status !== 204) {
				throw new Error(
					getApiErrorMessage(response.data, "Failed to delete user."),
				);
			}
		},
		onSuccess: async () => {
	...
	return (
		<section className="stack">
			...
			<form className="card stack" onSubmit={onSubmit}>
				...
				<div className="muted">
					Created {new Date(user.created_at).toLocaleString()} • Last updated{" "}
