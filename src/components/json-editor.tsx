...
function renderSyntaxErrorMessage(message: string, line: number | null, column: number | null): string {
...
export function JsonEditor({
  ...
  const dataValidation = useMemo(() => {
    if (mode !== "data" || parsed.kind !== "success" || !validationEnabled || validationSchema === undefined) {
      return null;
    }
  ...
  async function onFileChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Failed to read JSON file.";
  ...
  return (
    <div className="json-editor">
      ...
      <div className="json-editor-meta">
        {parsed.kind === "empty" ? <div className="muted">JSON field is empty.</div> : null}

        {parsed.kind === "error" ? (
          <div className="error-banner">
            {renderSyntaxErrorMessage(parsed.error.message, parsed.error.line, parsed.error.column)}
          </div>
        ) : null}
        ...
        {schemaAnalysis ? (
          <div className="json-editor-panel">
            ...
            {schemaAnalysis.issues.length === 0 ? (
              <div className="muted">Schema structure looks valid for the supported preview checks.</div>
            ) : (
        ...
        {mode === "data" && validationEnabled && validationSchema === undefined ? (
          <div className="json-editor-panel">
            <strong>Schema preview</strong>
            <div className="muted">
              This class validates object data, but no class schema is available in the current payload.
            </div>
          </div>
        ) : null}
        ...
        {dataValidation ? (
          <div className="json-editor-panel">
            <strong>Schema preview</strong>
            {dataValidation.note ? <div className="muted">{dataValidation.note}</div> : null}
            {dataValidation.issues.length === 0 ? (
              <div className="muted">Current JSON matches the available class schema preview.</div>
            ) : (
              <ul className="json-editor-list json-editor-list--errors">
