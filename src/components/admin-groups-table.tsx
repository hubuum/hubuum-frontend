	...
	const createMutation = useMutation({
		mutationFn: async (payload: NewGroup) => {
			...
			if (response.status !== 201) {
				throw new Error(
					getApiErrorMessage(response.data, "Failed to create group."),
				);
			}
		},
		onSuccess: async () => {
			...
			const results = await Promise.all(
				groupIds.map(async (groupId) => {
					...
					if (response.status !== 204) {
						throw new Error(
							`#${groupId}: ${getApiErrorMessage(response.data, "Failed to delete group.")}`,
						);
					}
				}),
			);
		...
		<div className="stack">
			...
			<div className="card table-wrap">
				<div className="table-header">
					...
					<div className="table-tools">
						...
						<button
							...
							disabled={
								deleteMutation.isPending || selectedGroupIds.length === 0
							}
						>
				...
				{memberCountsQuery.isError ? (
					<div className="muted">
						Could not load member counts.{" "}
						{memberCountsQuery.error instanceof Error
							? memberCountsQuery.error.message
							: ""}
					</div>
				) : null}
